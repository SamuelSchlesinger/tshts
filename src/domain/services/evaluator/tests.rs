// Test-setup style: the `Default::default() + sequential field assignment`
// pattern is easier to read for these many-field fixtures than a struct
// literal with `..Default::default()`. The `get(&k).is_some()` checks
// stay because they read naturally as "lookup succeeded" against a typed
// map; clippy prefers `contains_key`.
#![allow(
...
/// for safety) and that the domain layer's global HTTP fetcher is
/// wired to the production implementation. Tests can call this
/// freely — both calls are idempotent. Tests that want to verify
/// the gate's negative path override locally and restore at the end.
fn enable_network_for_test() {
    crate::infrastructure::fetcher::set_network_enabled(true);
    // `main()` wires this for the binary; lib tests have to do it themselves
    // or `GET()` would route through the trait's no-op default.
    crate::infrastructure::fetcher::install_as_http_fetcher();
}
...
#[test]
fn migrate_workbook_json_stamps_current_version() {
    // Missing/older version stamps up to current and reports source version.
    use crate::domain::models::{migrate_workbook_json, WORKBOOK_SCHEMA_VERSION};
    let mut raw: serde_json::Value =
        serde_json::from_str(r#"{"sheets":[],"sheet_names":[]}"#).unwrap();
    let from = migrate_workbook_json(&mut raw).unwrap();
    assert_eq!(from, 1, "files with no version field default to 1");
    assert_eq!(
        raw.get("version").and_then(|v| v.as_u64()),
        Some(WORKBOOK_SCHEMA_VERSION as u64),
        "migrator must stamp current version onto raw value",
    );
}

#[test]
fn migrate_workbook_json_rejects_future_version() {
    use crate::domain::models::migrate_workbook_json;
    let mut raw: serde_json::Value =
        serde_json::from_str(r#"{"version":9999,"sheets":[],"sheet_names":[]}"#).unwrap();
    assert!(migrate_workbook_json(&mut raw).is_err());
}

#[test]
fn migrate_workbook_json_rejects_non_integer_version() {
    use crate::domain::models::migrate_workbook_json;
    let mut raw: serde_json::Value =
        serde_json::from_str(r#"{"version":"oops","sheets":[],"sheet_names":[]}"#).unwrap();
    assert!(migrate_workbook_json(&mut raw).is_err());
}
